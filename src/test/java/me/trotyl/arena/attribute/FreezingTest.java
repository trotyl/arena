...
public class FreezingTest {
    ...
    public void apply_should_have_proper_result() {
        ...
        DamageRecord damage = freezing.apply(player1, player2, Attribute.normalAttack, Attribute.normalDefence);
    ...
    public void apply_should_have_proper_invocation_with_effect() {
        ...
        freezing.apply(player1, player2, Attribute.normalAttack, Attribute.normalDefence);

        InOrder inOrder = inOrder(player1, player2);
        inOrder.verify(player1).getAggressivity();
        inOrder.verify(player2).getDefence();
        inOrder.verify(player2).suffer(argThat(instanceOf(DamageRecord.class)), argThat(instanceOf(Freeze.class)));
    }
